...
export const PROJECT_STATUS_MAP = {
  ...
  evaluating: { text: '评审中', color: 'error' },
  // 成交确认阶段（ExpertProject 评审报告生成后回写 status='评审完成'；成交确认侧另以 awardStage 记录阶段）
  '评审完成': { text: '评审完成', color: 'purple' },
  'evaluation-done': { text: '评审完成', color: 'purple' },
  '已确认中选人': { text: '已确认中选人', color: 'cyan' },
  'winner-confirmed': { text: '已确认中选人', color: 'cyan' },
...
export const NEXT_STEP_MAP = {
  ...
  evaluating: { label: '评审大厅', description: '评审中，进入评审大厅查看进度' },
  // 成交确认阶段：评审完成→前往成交确认；已确认中选人→中选通知书；通知书已发→查看详情
  '评审完成': { label: '前往成交确认', description: '评审完成，前往成交确认确认中选人' },
  'evaluation-done': { label: '前往成交确认', description: '评审完成，前往成交确认确认中选人' },
  '已确认中选人': { label: '中选通知书', description: '已确认中选人，前往发送中选通知书' },
  'winner-confirmed': { label: '中选通知书', description: '已确认中选人，前往发送中选通知书' },
  '通知书已发': { label: '查看详情', description: '中选通知书已发出，查看项目详情' },
  'notice-sent': { label: '查看详情', description: '中选通知书已发出，查看项目详情' },
...
export const FLOW_NODES = [
  ...
  { key: 'doc', label: '编制采购文件' },
  { key: 'notice', label: '发布采购公告' },
  { key: 'bid', label: '上传响应文件/报价' },
  { key: 'opening', label: '线上开启' },
  { key: 'comparison', label: '线上比价' },
  { key: 'evaluation', label: '线上评审' },
  { key: 'award', label: '成交确认公示' }
]
...
export default function ProjectList() {
  ...
  const orgModeText = (mode) => ({ self: '自行采购', agent: '委托代理' }[mode] || mode || '-')
  ...
  const nextLabel = (row) => {
    // 代理草稿项目下一步为编制采购文件（代理已有创建权限但无编辑权限，agent-project-requirement-management-20260721）
  ...
  const publish = (row) => {
    Modal.confirm({
      title: '发布采购确认',
      content: `确认发布项目“${row.name}”？发布后将进入采购中状态并生成采购公告。`,
      okText: '确认发布采购',
      cancelText: '取消',
      onOk: () => {
        try {
          ...
          reloadProjects()
          addOperationRecord('发布采购', `项目“${row.name}”已发布采购，进入采购中状态`)
          message.success('发布采购成功，项目已进入「采购中」；下一步可前往「公告发布」发布采购公告')
        } catch {
  ...
  const nextStep = (row) => {
    ...
    if (row.status === 'draft') {
      // 代理草稿项目进入采购文件编制（代理无编辑权限，编辑仍限采购单位，与驾驶舱 getAgentActions 一致）
      if (isAgentRole) {
    ...
    // 成交确认阶段（评审报告生成后 status='评审完成'；兼容 awardStage 键值口径）
    // 代理职责为汇总评审报告提交成交审批（approval-center），采购单位进入成交确认确认
    if (['评审完成', 'evaluation-done'].includes(row.status)) {
      navigate({ to: isAgentRole ? '/admin/approval-center' : '/admin/award-confirm', search: { projectId: row.id } })
      return
    }
    if (['已确认中选人', 'winner-confirmed'].includes(row.status)) {
