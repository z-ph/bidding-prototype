...
const { pageReviews, addReview, deleteReview, updateReview, clearPageReviews, exportToJSON, exportToMarkdown } = useReview()
...
function captureEnv() {
  return {
    ...
    pageName: route.meta?.title || route.name || route.path
  }
}
...
function onElementClick(e) {
  if (mode.value !== 'element' || formVisible.value || isDragging.value) return
  // 先判断是否为页面元素，避免拦截工具栏/弹窗等评审 UI 的点击
  const target = getSafeTarget(e)
  if (!target) return
  e.preventDefault()
  e.stopPropagation()
...
function handleOverlayClick(e) {
  // 仅用于阻止事件穿透，实际选择逻辑由 document 监听处理
}
...
.review-overlay > *:not(.review-hint):not(.highlight-box):not(.drag-rect) {
  pointer-events: auto;
}
